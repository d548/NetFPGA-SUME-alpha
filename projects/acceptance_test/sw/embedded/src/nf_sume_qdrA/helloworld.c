 ...
 *
 * File:
 *     sw/embedded/src/nf_sume_qdrA/helloworld.c
 *
 * Project:
 *     acceptance_test
 *
...
void runAutoTest(void) {
	XStatus status;
	status = qdrRwTest();
	printTestResult("QDRA_RW", status);
}
...
void runManualTest(void) {
	...
	while(1) {
		...
		xil_printf("s: Read QDR Test Status\r\n");
...
int main()
{
	...
	xil_printf("NetFPGA-SUME qdrA Acceptance Test\r\n");
