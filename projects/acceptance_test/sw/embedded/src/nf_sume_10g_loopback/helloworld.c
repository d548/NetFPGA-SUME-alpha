 ...
 *
 * File:
 *     sw/embedded/src/nf_sume_10g_loopback/helloworld.c
 *
 * Project:
 *     acceptance_test
 *
 ...
 *
 * Description:
 *     main function for 10G Loopback test
 *
...
void runAutoTest(void) {
	XStatus status;
	status = xge_auto_test(); 
	printTestResult("10G_Loopback", status);
}
...
void runManualTest(void) {
	...
	while(1) {
		...
		xil_printf("s: Read 10G Loopback Test Status\r\n");
...
int main()
{
	...
	xil_printf("NetFPGA-SUME QTH Acceptance Test\r\n");
	...
	Status = config_SI5324();
	if (Status != XST_SUCCESS) {
		xil_printf("Si5324 Initialization FAILED\n\r");
		return XST_FAILURE;
	}
