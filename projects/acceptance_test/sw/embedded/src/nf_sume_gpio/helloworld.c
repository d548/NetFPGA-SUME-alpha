 ...
 *
 * File:
 *     sw/embedded/src/nf_sume_gpio/helloworld.c
 *
 * Project:
 *     acceptance_test
 *
...
void runAutoTest(void) {
	XStatus status;
	status = sdcard_auto_test();
	printTestResult("SD_Card", status);

	status = fmc_clk_auto_test();
	printTestResult("FMC_Clocks", status);
	
	status = gpio_auto_test();
	printTestResult("GPIO_Test", status);
}
...
void runManualTest(void) {
	...
	while(1) {
		...
		xil_printf("c: Read Clock Info\r\n");
		xil_printf("d: Read SD Card Info\r\n");
		xil_printf("g: GPIO Test Info\r\n");
...
int main()
{
	...
	xil_printf("NetFPGA-SUME GPIO/SD/FMC_Clk Acceptance Test\r\n");
