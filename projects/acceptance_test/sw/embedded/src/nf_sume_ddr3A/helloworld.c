 ...
 *
 * File:
 *     sw/embedded/src/nf_sume_ddr3A/helloworld.c
 *
 * Project:
 *     acceptance_test
 *
...
void runAutoTest(void) {
	XStatus status;
	status = ddr3IicTest('A');
	printTestResult("DDR3A_IIC", status);

	status = ddr3RwTest();
	printTestResult("DDR3A_RW", status);
}
...
int main()
{
	...
	xil_printf("NetFPGA-SUME ddr3A Acceptance Test\r\n");
