 ...
 *
 * File:
 *     sw/embedded/src/nf_sume_fmc_gtwizard/fmc_gtwizard_test.c
 *
 * Project:
 *     acceptance_test
 *
 ...
 *
 * Description:
 *     Read FMC GTwizsard Test Status
 *
...
u32 arGpioDevIds[10] = {
	...
	XPAR_AXI_GPIO_RO_9_DEVICE_ID,
	XPAR_AXI_GPIO_RO_10_DEVICE_ID
};
...
int fmcRwStat() {
	...
	fmcStatus = XGpio_DiscreteRead(&gpioRO0_status, 1);
	if(fmcStatus != 0x01) {
		xil_printf("fmc: Transceiver Initialization Failed\r\n");
		return XST_FAILURE;
	}

	for(i = 0; i < 8; i++) {
		fmcStatus = XGpio_DiscreteRead(&gpioROs[i], 1);
		if(fmcStatus != 0x00) {
			xil_printf("fmc-Channel%d: Error Count %4d (0x%02x)\r\n", i, fmcStatus, fmcStatus);
			errStatus = XST_FAILURE;
		}
	}
	
	if(errStatus == XST_SUCCESS) {
		xil_printf("fmc: Test Passed with no Errors\r\n");
	}
