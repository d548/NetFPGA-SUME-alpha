...
set repo_dir $::env(NF_DESIGN_DIR)/hw/ip_repo
...
set_property -dict [list CONFIG.C_HIGHADDR $OUTPUT_PORT_LOOKUP_HIGHADDR] [get_ips output_port_lookup_ip]
...
set_property -dict [list CONFIG.C_HIGHADDR $INPUT_ARBITER_HIGHADDR] [get_ips input_arbiter_ip]
...
set_property -dict [list CONFIG.C_HIGHADDR $OUTPUT_QUEUES_HIGHADDR] [get_ips output_queues_ip]
...
#set_property generate_synth_checkpoint false [get_files axis_sim_record_ip.xci]
...
#set_property generate_synth_checkpoint false [get_files axis_sim_stim_ip.xci]
#reset_target all [get_ips axis_sim_stim_ip]
...
###add microblaze subsystem
##source $::env(NF_DESIGN_DIR)/hw/tcl/mbsys.tcl
##create_bd -name nf_sume_mbsys -iic true -uart true 

source $::env(NF_DESIGN_DIR)/hw/tcl/control_sub_sim.tcl

##read_verilog "$::env(NF_DESIGN_DIR)/hw/hdl/nf_sume_microblaze.v"
