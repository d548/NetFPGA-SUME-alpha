...
#define	DRIVER_NAME		"NetFPGA SUME"
...
sume_start_xmit(struct sk_buff *skb, struct net_device *netdev)
{
	...
	/*
	 * Current [some SUME/NF10 converter block] cannot handle
	...
	/* Stupids padto doesn't update the length, just [allocs] zeros bits. */
	if (skb->len < ETH_ZLEN)
		skb->len = ETH_ZLEN;
#endif
	...
	*(p32 + 3) = cpu_to_le32(0);	/* Timestamp. */
	...
	/* Let the FPGA know about the ranfer. */
	offset = 0;
	last = 1;
...
sume_poll_controller(struct net_device *netdev)
{
	 ...
	 * XXX-BZ but really want to have interrupt generation dsiabled.
...
/* Linux shuld really have this wrapper function. */
static int __inline
...
sume_rx_build_skb(struct sume_adapter *adapter, int i, unsigned int len)
{
	...
	if ((16 + sume_16boff * sizeof(uint16_t) + plen) > len ||
		...
		printk(KERN_INFO "%s: corrupted packet (16 + %d > %d, magic = "
		    "0x%04x)\n", __func__, plen, len, magic);
		return (-EINVAL);
	}
    ...
    unsigned int vect1)
{
	 ...
	 * to quickly lokup for which channel(s) we got an interrupt?
	 */
	for (i = 0; i < adapter->num_chnls; i++) {
		...
		SUME_LOCK_TX(adapter, i, flags);
		/*
		 * Check any TX completions first, as RX could trigger more TX.
		 * Note: RIFFA terminlogy thinks in different directions.
		 */
		/* XXX-BZ loads of magic numbers following. */
		/* RX (PC send) scatter gather buffer is read. */
		if (vect & (1 << ((5 * i) + 3))) {
			if (adapter->send[i]->state !=
			...
			adapter->send[i]->state = SUME_RIFFA_CHAN_STATE_READ;
		}
		if (vect & (1 << ((5 * i) + 4))) {
			if (adapter->send[i]->state !=
			...
			adapter->send[i]->state = SUME_RIFFA_CHAN_STATE_LEN;

			len = read_reg(adapter,
			    RIFFA_CHNL_REG(i, RIFFA_RX_TNFR_LEN_REG_OFF));
			/*
			 * XXX-BZ should compare length with expected amount of
			 * data transfered; only on match advance state?
			 */
			if (i == SUME_RIFFA_CHANNEL_DATA(adapter))
				adapter->send[i]->state =
				    SUME_RIFFA_CHAN_STATE_IDLE;
			else if (i == SUME_RIFFA_CHANNEL_REG(adapter))
				wake_up_interruptible(&adapter->send[i]->waitq);
			else
				printk(KERN_WARNING "%s: interrupt on ch %d "
				    "unexpected in send+4 state %u: "
				    "vect=0x%08x\n", __func__,
				    i, adapter->send[i]->state, vect);
		}
		SUME_UNLOCK_TX(adapter, i, flags);
		...
		if (vect & (1 << ((5 * i) + 0))) {
			...
			adapter->recv[i]->offlast = read_reg(adapter,
			    RIFFA_CHNL_REG(i, RIFFA_TX_OFFLAST_REG_OFF));
			adapter->recv[i]->len = read_reg(adapter,
			    RIFFA_CHNL_REG(i, RIFFA_TX_LEN_REG_OFF));

			/* Boundary checks. */
			max_ptr = (unsigned long)(adapter->recv[i]->bouncebuf +
			    SUME_RIFFA_OFFSET(adapter->recv[i]->offlast) +
			    SUME_RIFFA_LEN(adapter->recv[i]->len) - 1);
			if (max_ptr < (unsigned long)adapter->recv[i]->bouncebuf)
			{
				printk(KERN_INFO "%s: receive buffer "
				    "wrap-around overflow.\n", __func__);
				/* XXX-BZ recover? */
			}
			if ((SUME_RIFFA_OFFSET(adapter->recv[i]->offlast) +
			    SUME_RIFFA_LEN(adapter->recv[i]->len)) >
			    adapter->recv[i]->bouncebuf_len) {
				printk(KERN_INFO "%s: receive buffer too "
				    "small.\n", __func__);
				/* XXX-BZ recover? */
			}
			...
			error = sume_riffa_fill_sg_buf(adapter,
			    ...
			    SUME_RIFFA_LEN(adapter->recv[i]->len));
			if (error != 0) {
				printk(KERN_INFO "%s: Failed to build S/G "
				    "map.\n", __func__);
				/* XXX-BZ recover? */
			}
			write_reg(adapter,
			    RIFFA_CHNL_REG(i, RIFFA_TX_SG_ADDR_LO_REG_OFF),
			    (adapter->recv[i]->buf_hw_addr & 0xFFFFFFFF));
			write_reg(adapter,
			    RIFFA_CHNL_REG(i, RIFFA_TX_SG_ADDR_HI_REG_OFF),
			    ((adapter->recv[i]->buf_hw_addr >> 32) & 0xFFFFFFFF));
			write_reg(adapter,
			    RIFFA_CHNL_REG(i, RIFFA_TX_SG_LEN_REG_OFF),
			    4 * adapter->recv[i]->num_sg);

			adapter->recv[i]->state = SUME_RIFFA_CHAN_STATE_READY;
		}
		...
		if (vect & (1 << ((5 * i) + 1))) {
			...
			adapter->recv[i]->state = SUME_RIFFA_CHAN_STATE_READ;
		}
		...
		if (vect & (1 << ((5 * i) + 2))) {
			...
			len = read_reg(adapter,
			    RIFFA_CHNL_REG(i, RIFFA_TX_TNFR_LEN_REG_OFF));
			/* XXX-BZ compare len to expected len? */

			/* Remember, len and recv[i]->len are words. */
			if (i == SUME_RIFFA_CHANNEL_DATA(adapter)) {
				error = sume_rx_build_skb(adapter, i, len << 2);
				adapter->recv[i]->state =
				    SUME_RIFFA_CHAN_STATE_IDLE;
			} else if (i == SUME_RIFFA_CHANNEL_REG(adapter))
				wake_up_interruptible(&adapter->recv[i]->waitq);
			else
				printk(KERN_WARNING "%s: interrupt on ch %d "
				    "unexpected in recv+2 state %u: "
				    "vect=0x%08x\n", __func__,
				    i, adapter->recv[i]->state, vect);
...
sume_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	snprintf(adapter->name, sizeof(adapter->name), "%s%d", pci_name(pdev),
	     PCI_SLOT(pdev->devfn));
...
MODULE_DESCRIPTION(DRIVER_NAME "network driver");
